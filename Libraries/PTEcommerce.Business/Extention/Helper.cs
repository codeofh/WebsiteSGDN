...
namespace PTEcommerce.Business
{
    public class Helper
    {
        ...
        public static bool ValidateOTP(string otp, string secretKey)
        {
            Totp totp = new Totp(Encoding.ASCII.GetBytes(secretKey), 1800, OtpHashMode.Sha256, 6);
            long outTime = 0;
            if (!totp.VerifyTotp(otp, out outTime))
            {
                return false;
            }
            return true;
        ...
        {
            return m.ToString("N", new CultureInfo("vi-VN"));
        }
