...
namespace marketplace
{
    public class IplAccountCustomer : BaseService<AccountCustomer, int>, IAccountCustomer
    {
        ...
        public AccountCustomer ViewDetailByPhone(string phone)
        {
            try
            {
                var p = new DynamicParameters();
                p.Add("@Phone", phone);
                var data = unitOfWork.Procedure<AccountCustomer>("[AccountCustomer_ViewDetailByPhone]", p).FirstOrDefault();
                return data;
            }
            catch (Exception ex)
            {
        ...
        public List<AccountCustomer> GetListPartnerByIdAccount(int idAccount)
        {
            try
            {
                var p = new DynamicParameters();
                p.Add("@IdAccount", idAccount);
                var data = unitOfWork.Procedure<AccountCustomer>("AccountCustomer_GetListReferal", p).ToList();
                return data;
            }
            catch (Exception ex)
            {
        ...
        public List<AccountCustomerExtend> GetListPartnerReportByIdAccount(int idAccount)
        {
            try
            {
                var p = new DynamicParameters();
                p.Add("@IdAccount", idAccount);
                var data = unitOfWork.Procedure<AccountCustomerExtend>("[AccountCustomer_GetListReferalReport]", p).ToList();
                return data;
            }
            catch (Exception ex)
            {
